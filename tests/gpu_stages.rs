...
fn create3_finds_and_verifies_low_threshold() {
    use nullforge::gpu::create3_deployed_address;
    use nullforge::miner::create3::DEFAULT_CREATE3_PROXY_HASH;

    let ctx = MetalContext::new();
    let factory: [u8; 20] = [0x11; 20];
    let proxy_hash: [u8; 32] = DEFAULT_CREATE3_PROXY_HASH;
    ...
    for h in &hits {
        ...
        // Independent cross-check against the pure host two-hop derivation.
        let want = create3_deployed_address(&factory, &proxy_hash, &h.salt);
        assert_eq!(want, h.address, "CREATE3 address != host reference");
        assert!(h.address[0] >> 4 == 0, "claimed leading zero nibble wrong");
    }
}
